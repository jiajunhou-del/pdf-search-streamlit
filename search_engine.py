...
class SearchIndex:
    ...
    def search(self, query: str, top_k: int = 8, doc_id: str = None):
        ...
        # A single page can be split into several overlapping chunks (see
        # pdf_processor.chunk_pages), so the raw ranking can contain more
        # than one hit for the same (doc_id, page_number) -- keep only the
        # highest-scoring chunk per page, both because showing the same
        # page twice isn't useful and because the UI keys results by
        # (doc_id, page_number).
